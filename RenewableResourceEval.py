...
def Run_RenewableResourceEval_V3(analysisTypes, analysisOptions, databases, namingConv):
    # -------------
    # Unpack Inputs
    ...
    if skipExclusionZone != "Y":
        ...
        if usePopulation == "Y":
            ...
            # Process: Dissolve
            arcpy.Dissolve_management(population_buffer, population_buffer_dissolve, "", "", "MULTI_PART",
                                      "DISSOLVE_LINES")

            # Process: Polygon to Raster
            arcpy.PolygonToRaster_conversion(population_buffer_dissolve, "OBJECTID", population_bufferToR,
                                             "CELL_CENTER", "NONE", "18000")
        ...
        # Water
        expr = arcpy.sa.RemapValue(
            [[0, 0], [11, 1], [21, 0], [22, 0], [23, 0], [24, 0], [31, 0], [41, 0], [42, 0], [43, 0], [51, 0], [52, 0],
             [71, 0], [72, 0], [74, 0], [81, 0], [82, 0], [90, 0], [95, 0]])
        temp = arcpy.sa.Reclassify(LULC_project, "Value", expr, "DATA")
        temp.save(Excl_Water)

        # Cities / Urban Areas
        expr = arcpy.sa.RemapValue(
            [[0, 0], [11, 0], [21, 0], [22, 1], [23, 1], [24, 1], [31, 0], [41, 0], [42, 0], [43, 0], [51, 0], [52, 0],
             [71, 0], [72, 0], [74, 0], [81, 0], [82, 0], [90, 0], [95, 0]])
        temp = arcpy.sa.Reclassify(LULC_project, "Value", expr, "DATA")
        ...
        # Crops
        expr = arcpy.sa.RemapValue(
            [[0, 0], [11, 0], [21, 0], [22, 0], [23, 0], [24, 0], [31, 0], [41, 0], [42, 0], [43, 0], [51, 0], [52, 0],
             [71, 0], [72, 0], [74, 0], [81, 0], [82, 1], [90, 0], [95, 0]])
        temp = arcpy.sa.Reclassify(LULC_project, "Value", expr, "DATA")
        temp.save(Excl_Crops)

        # Forest
        expr = arcpy.sa.RemapValue(
            [[0, 0], [11, 0], [21, 0], [22, 0], [23, 0], [24, 0], [31, 0], [41, 1], [42, 1], [43, 1], [51, 0], [52, 0],
             [71, 0], [72, 0], [74, 0], [81, 0], [82, 0], [90, 0], [95, 0]])
        temp = arcpy.sa.Reclassify(LULC_project, "Value", expr, "DATA")
    ...
    if analyzeSolar == "Y":
        ...
        if useProtect == "Y":
            print "useProtect == Y"
            if usePopulation == "Y":
                temp = Raster(Incl_Population) - (
                            Raster(Excl_Slope) | Raster(Excl_Water) | Raster(Excl_Cities) | Raster(Excl_Crops) | Raster(
                        Excl_Forest) | Raster(Excl_Protect))
            else:
                temp = 1 - (Raster(Excl_Slope) | Raster(Excl_Water) | Raster(Excl_Cities) | Raster(Excl_Crops) | Raster(
                    Excl_Forest) | Raster(Excl_Protect))
        else:
            if usePopulation == "Y":
                temp = Raster(Incl_Population) - (
    ...
    if analyzeWind == "Y":
        ...
        if useProtect == "Y":
            ...
            if usePopulation == "Y":
                temp = Raster(Incl_Population) - (
                            Raster(Excl_Slope) | Raster(Excl_Water) | Raster(Excl_Cities) | Raster(
                        Excl_Forest) | Raster(Excl_Protect))
            else:
                temp = 1 - (Raster(Excl_Slope) | Raster(Excl_Water) | Raster(Excl_Cities) | Raster(
                    Excl_Forest) | Raster(Excl_Protect))
        else:
            if usePopulation == "Y":
                temp = Raster(Incl_Population) - (
                            Raster(Excl_Slope) | Raster(Excl_Water) | Raster(Excl_Cities) | Raster(Excl_Forest))
            else:
        ...
        temp = Con(Raster(Wind_Full_Load_Hours) < min_range, 0,
                   Con(Raster(Wind_Full_Load_Hours) > max_range, max_range, Raster(Wind_Full_Load_Hours)))
